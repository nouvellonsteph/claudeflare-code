...
- **VPC egress**: Direct internet access is disabled. Container HTTP requests and raw IPv4/IPv6 TCP connections on every port use the authenticated Access email as their Cloudflare Gateway runtime identity. Preserving TLS and SNI lets Gateway apply network and HTTP policies without Containers terminating TLS first.
